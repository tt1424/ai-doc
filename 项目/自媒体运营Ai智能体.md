...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=ZTk5ZGQ4ZDhhNTQ2OGRkNmU0NGNjZmUwNTI5YzQ5YTRfOE1keENEN1ZBeXk2dXVDMDlqVDduYkYzNzBTbGwyRm9fVG9rZW46WGpXQ2I4Vk94b0NFd0h4dTduNGNvbG1xbjRkXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
- **表结构:** LangGraph 库会自动创建 `checkpoints` 表 (thread_id, checkpoint_id, state_data)。

1. **业务数据存储 (Business Tables):** 用于前端列表展示、历史归档。

- **Table:** **`content_projects`**
  - `id`: UUID
  - `user_id`: 操作员ID
  - `thread_id`: **关键字段**，关联 LangGraph 的 thread_id
  - `topic`: 最终确定的选题
...
- **Action:** 创建 thread_id，运行 Graph 直到遇到第一个 `interrupt` (即生成完选题后暂停)。
- **Return:** `thread_id`, `next_step`: "human_select", `payload`: [选题A, 选题B, 选题C]
...
1. **AI 服务抽象化 (****`services/`****):**
   1. 目前你可能用 OpenAI 生成文案。未来如果有针对编程更好的模型（如 DeepSeek Coder），你只需修改 `llm_service.py`，LangGraph 的逻辑层不需要动。
2. **图片生成的解耦:**
   1. `extract_visual_points` 节点只负责生成 "Prompt"。
   2. `generate_images` 节点负责调 API。如果未来需要生成的不是图片而是**技术代码截图**（Carbon 等工具），只需替换这个节点的具体实现，流程不变。
3. **多渠道适配:**
   1. 小红书需要图片+短文，公众号需要长文。
   2. 在 Graph 中可以设计并行分支：文案确认后，分支 A 去生成公众号排版格式，分支 B 去做小红书图片摘要。
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=MTQ3MjUxMGQ0YWJlNDlhYWIwNjYxNTY3YzRiNDFjODFfRUNHNnladlNPQWJVYXFQOU9DeHlWSGFERnk0b0VpcDRfVG9rZW46WXVCN2JiVlFhb3hCMWR4T0ppT2NLcE9hbnNkXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

# 第五步  start   开始Agent接口，进入选题审核

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=ZjE4ODAzMDQ3OWVlYmJmYzM4OWQzMTdiYzkwYWNjMGJfNXhoaVFubFI1cHd1RWpkcXRta3N1VUdFTEV4NUNLcHdfVG9rZW46RlBlRGJsbmxibzdUQ0d4WUF3NmNFNGM5bnJnXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
thread_id： 调用start接口，python服务端生成的随机字符串

status： graph.invoke  在node节点中 return的数据改的

generated_topics： 先是在node节点中LLM生成的，然后return改的

message：手动构造的

interrupt_info： 在node节点human_select_topic，调用interrupt生成的

# 第六步  "/resume/{thread_id}" 进入文章草稿审核流程
...
注意： 我们在human_review_node人工介入的，不管同意还是拒绝，human_review_node会再重新走一次！！！

当拒绝之后，流程会重新回到write_draft_node，带上修改意见，让LLM重新生成。
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=NjBmNDcwMGNlMjg4MDBlM2JhMTg0ZWM2NTA5ZDgzYWJfeHFKRERRQzBMQ3JXMFVCNDBNTWpXaUNhSEQxbFFtdTBfVG9rZW46UkFiS2I2Mkdvb2JPSFF4MVlkdmNCd0QzbkdmXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
相当于异步执行_generate_single_image_sync
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=OTI1NGFmOGEwMTkxZTQwOTlkNTNjOGJhOTc2MTFhMDBfbWNIM1RIU21nNUFqd1R6S3dqZ2RXVWV2MVo1UlZ5MXFfVG9rZW46VmxONGIybDNmbzRDQzV4dTUxTmNBVmdibmdnXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
1. 使用langgraph官方提供的AsyncPostgresSaver创建连接池 ，返回值就是_checkpointer
2. 将_checkpointer和graph关联
3. graph运行自动写入checkpointer
4. 读取的，也是根据config从graph中查询graph.aget_state
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=OTQ0MzIzODBlYjU1ZWVlYzcwY2M1NTZhYTM2Njg1MzlfbWJkcDVDd1JhbVdSa1NTSlF0bFV3WEFpNHZUVWFzQjFfVG9rZW46VVJuT2J3SXJmb083VXR4SVRBY2NSeE56bkJnXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

## 进入pgAdmin4

初始密码：123456

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=M2NmZmI4YmYxNTdmNjFiY2YzMmE3NzFhM2ExOTRmZTlfaVJCeXFzOEJyV1NraXVlWlZROEtoWTVUN0w2alNtNlhfVG9rZW46SkpIemJNWjJ5bzZISDd4ZklrRGNNTXdXbjlnXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

创建数据库langgraph_db

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=M2Q4NzQ3NGY1OTQxN2Y3MmJkMGI5ZGMxZmMzN2ExMDZfM1NTc291Q1FsUmNVd25ONzFoNnRmbllwYm5yVDBZcW1fVG9rZW46QTkwZ2JZTzd2b1VmRGh4bTVTMWNvOXBYbklnXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

## 重启电脑之后数据库可能连不上，解决办法

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=N2RiOGRhYmYwNWRiNWMzMjQ1NzlkNDAwOTE4ZDUxMWRfemRqY0ppSUZlNTB4S2N1ZXJLcG51T2FsUEJOUThoMnNfVG9rZW46RGVxeWJNU2xxb2xrRmx4WVdSVGNIQlRhbndmXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
- 第24行：通过环境变量 USE_MOCK_CHECKPOINTER 控制持久化模式
- true → 使用内存存储（MemorySaver），数据不持久化
- false（默认）→ 使用 PostgreSQL 持久化
- 第27-29行：定义两个全局变量
- _checkpointer：Checkpointer 单例实例
- _connection_pool：PostgreSQL 连接池（仅 PostgreSQL 模式使用）
...
| 表名                  | 用途             |
| --------------------- | ---------------- |
| checkpoints           | 保存每次状态快照 |
| checkpoint_writes     | 保存节点写入记录 |
| checkpoint_migrations | 版本迁移记录     |

每条记录通过 (thread_id, checkpoint_id) 唯一标识。
...
| 表名                  | 用途         | 存储内容                                       |
| --------------------- | ------------ | ---------------------------------------------- |
| checkpoints           | 检查点元数据 | thread_id, checkpoint_id, 父检查点ID, 元数据等 |
| checkpoint_blobs      | 状态数据存储 | 实际的状态值（序列化后的二进制/JSON数据）      |
| checkpoint_writes     | 写入记录     | 每个节点的写入操作记录                         |
| checkpoint_migrations | 版本迁移     | Schema 版本控制                                |

## checkpoint_blobs 表详解
...
2. checkpoint_blobs 表：存储可能很大的状态数据
...
### checkpoint_migrations 表 - Schema 版本
...
原因：数据是用 pickle 序列化后存成二进制的！当你直接查询 checkpoint_blobs 表时：
...
调用你项目的 /workflow/state/{thread_id} 接口：

curl http://localhost:8000/api/v1/workflow/state/你的thread_id

返回的 values 字段会包含 generated_topics 列表。
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=MmFjYTEyNTU4MjYyZjcxYTQ1NzA0M2YwZWVjY2NmNzRfTndGWWRyR3BUMWxyaU1hdzNqUnNoTlBzTlFtcUZiZEZfVG9rZW46SDNTTWJReHZ0b2VtV2t4UFhvRWNGR2JLbkdoXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
1. 实现llm_mock、image_mock、checkpointer_mocker
...
相当于_generate_single_image_sync放到另一个线程去跑，跑完再回到主线程，模拟异步
...
1. 服务端处理请求  @router.post("/stream/start")
...
但 LangGraph 的 astream_events() 仍能捕获到 token 流，因为：

1. LangChain 的 ChatOpenAI 默认支持流式
2. astream_events() 会自动监听底层的流式事件

graph.astream_events会把LLM的底层调用变成astream模式，但是还是建议显性的把LLM的调用变成stream模式。方便项目维护，一目了然。
...
- 使用 Pydantic + LangChain 的 with_structured_output
- 强制 LLM 输出符合 Schema
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=ZDJjZWQyMGRhZDc3Zjg0OTIyZTliNmM2ZDhhNDAyYWVfQWhzUTBzaWZxODBhd3B0MU05NmQzYWUxWDZlYmpRckNfVG9rZW46RXhtQ2JPUlF4b0dla0p4S2xpTGNhQmhYbkl5XzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
原因：with_structured_output() 会构建 llm | output_parser 链，最终返回的是解析后的 Pydantic 对象，而不是原始 AIMessage，所以拿不到其中的 usage_metadata。解决方式：不再使用 with_structured_output，改为：

1. 使用 JSON 模式（response_format={"type": "json_object"}）直接调用 LLM
2. 得到原始 AIMessage，从中提取 usage
3. 手动解析 response.content 为 JSON，再用 Pydantic 校验

已实现：

- 新增 llm_json 属性，使用 model_kwargs={"response_format": {"type": "json_object"}}
- plan_topics 和 extract_visual_points 改为通过 llm_json 调用，并手动解析 JSON
- 若 Doubao 不支持 response_format 或解析失败，会回退到按行解析，保证可用性
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=YWU5ZmQxNWNiYzM1N2YwMThmYjRlNWY4NDQ3YjQ4Y2VfMlVoWGJJYlhnZmlYdWk4RlZHSXJsdEwzazl4amJuN0dfVG9rZW46WVRMcWJWamhEbzhSdHF4VjJzVWNwcUJNbnhoXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=YTQ1NTEyMDA5ZTYwODA4ZTU0Zjg3MjE5MWQ5ZmMxZDVfTjdVcmgwU0pqQ2l6TjAxVjJEZ0R0T3M5MllwMk81SzVfVG9rZW46VEFjUGJienFHb2ZISUN4eHNFaGNrdTcybjhkXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=NTI1MmMzMmM2M2M4NmVhZjY2YWI4ODBlYzhiZmIwNzlfSGZMWHJVOHFXdEtSTUtWOUZTTUtTc2Z3VVNUR004c2NfVG9rZW46UkRUbWJNU2pnbzhaQ0x4M0liQWNIem9ObkhoXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
### 1. 图片生成服务 (app/services/image_service.py)
...
- generate_image(prompt, size, style) → 生成单张图片
- generate_images_batch(prompts, size, style) → 批量生成图片
...
- chat_completion(messages, model, stream) → 对话补全
- structured_output(messages, schema) → 结构化输出（返回JSON）
- plan_topics(topic, style, count) → 生成内容选题
- write_article(topic, outline, style) → 生成文章
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=MWJhNTBkYWJkMGQ1YTAyYjFmYTA1MjJmMjIxMmVlNmRfZWdGVnV2Tlp2RnVkalFGcFVjN3loeGhEMUZOTEdKZXpfVG9rZW46UkpPUWJnclNJbzVubVJ4cnpmemM1cEFTbjhjXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=NGE0ZTMwY2Y0MDY5MTE4YjBlY2EzNzA2MzdkZGYzZDZfWlBmcm9FM1pxN2FHZkhSZUVVQ0tvR29nSWppTnJwdGRfVG9rZW46TkRyaGJEZ2c5b2VuNGp4QThYMmNEaEVsbnJnXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=NjJlODM5ODBlMWMxYWI5ZDJiMzhjOWJkNmY0NzZiNDlfWXdnUHJDTTI3UEdhS3J2bzlzcDVhS3FZakNveDNMNmdfVG9rZW46UXN6OWJES2w1b2NVTGJ4ZjZrU2MzcjdCbjUxXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=MGM3Y2E1NzUzYjYzMWQxZmNkODA5NGIxOWEyNWIyOWRfMWxyVWdtY2RmbVZYMGlUMXo2NGxpelZoZ25uMzhCeVRfVG9rZW46RExWeGJpVTF5b2Z0UXh4NzdSbmNLemNibmxkXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=OWU5YzRjYzU3ZmIyOTM0ZDFiZDg1ODFmMzVjZmNhNzdfcWZzejhnRkNWdlo0cFB0S1IxYnVqTmdTbWVGcnFwTzlfVG9rZW46WEc4cmJBTmJNb0Vibzl4dXZUZmN5UWhIbjJkXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=ZGI3Yjg4ZmZkZjBjMzI1YzRkN2IwOTMxNmY4MDdmNGNfRmpwWXdFNURNRGFlb2F2MTFvbk1xMVMzQXlCRDF5QTdfVG9rZW46WDQ4WGJiTmtrb05WNlJ4b1V6Y2NDU0J4bm9lXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

## LLM调用性能优化

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=ZTdlNDBiZjU2N2NmZjkwNGY1NWNmMjcyNTQwZTg0MjVfY1VlWnhhZkhZV1d5QWVyamhUUkJidW5jUTdrcHBEMWxfVG9rZW46S2E5WWJwdklHb2l2TXJ4V0x2RmNIVGg5blVoXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

生成选题调整 temperature为0合适吗？为什么

这是个好问题。选题生成用 temperature=0 其实不太合适。

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=MzJhZWYwNjcxNTJlODg3NmVjNjVhZWY0N2I1YzM2YTBfYlJWQnBzSFdYTXRieHpxZlJlTGlmck9WbGUyR2hKQWxfVG9rZW46VWRpYWJRbkJJb3JZdTF4d05YTWNLMnRVbkplXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

## 当前设置的问题

当前 LLM_TEMPERATURE=0，意味着：

- 同一个主题方向，每次生成的选题几乎一样
- 缺乏多样性，用户重试也得不到新选题
- 对于创意类任务不理想

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=ZDliMGJhMTMyMWMxZDU0YWEwYmFiYTMyMzZiZmViMWVfNmVRbTI3QnMzdXZsYTJUdXBWd3l5ZUU5S3dTUHN1QkRfVG9rZW46T3ZmWWI1UGplb3daN0N4eXRNYmNET0gzbmRoXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

## 最终效果

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=YTYzODIyMWViZjkzMTdlNmI0OGRhYzkyMGYzNDA0NmZfWTE2ckJTUGtJZHFaQ0hTTXlEZEJoWWxRZzNsbW8xdnhfVG9rZW46WUZIemJPYndab1VjRDl4eU54RGMwMnZGbnpiXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

优化前

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=YWViZjk2MzNjNzZkNWIwNTE5NzdmOWUxN2NhOTIyMTBfUmtqYVB0V2RWRFRVZlRuajdHWGJXN0ZNUklPaDRwZDVfVG9rZW46TFFKd2JQMFB1b0pYbW54TjVIQmN6c21GblFnXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

优化后

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=Y2EyN2I5NzRjZGZkMmUyZTNkY2E2Nzg0MmE3NTFkNWRfSEdCWVNNV1VjNkRmR1k4OTM1OTYyaG9ZSnZ3bklEZndfVG9rZW46Uzdsd2IzSTNqb0hTYjB4Mml6cWNKb2VFbnJjXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)

![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=ODA4OWY2OGIzODNlYzJhMWIyMzQxNjUyMDIyZWNmYjVfSlVMUXJEMHpISklVMDRpSE5Wc2t3UjFjWUh4c2hkY3dfVG9rZW46SVlRd2JDcExlbzRVdDZ4c3VyTGNjend4bkllXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
key：sk-cLqcKKdysU6AhdT0nFF8GPvs57WqRmxSsYnr0ES7vhvdKIaf
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=MTkyYTM2MmQ0YmFjNjBiYjdiY2E2ZGIxNTY5ODhhMmVfcVVDVGZmUzI1eU15WlA3YkNpZ0xYTFNSeVVUTFVmcHlfVG9rZW46RUlvYWJQUzc0b3ZGYjR4akx1SGNpTWRLbjJlXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=M2U0ZWUxZjM3ZmI0YWVlMzRhMWE2ZjQ0OGE1M2JiNzJfQWRLV0RMNlBTWm9tS3dhRWZoaHdXamdSWFB5ZmFaTUxfVG9rZW46Q0NScmJoZU9kb0VLZmd4ZzdYT2NDSlU1bk9lXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
![img](https://jianxuanguan.feishu.cn/space/api/box/stream/download/asynccode/?code=MGU4MmJkMjA5MzgzNDIyMTNjMDgyYWQ1MjIwMzNhZGVfa1JxZGFpczNsOUNUUDd3bFBIUFFkWlA0SE81QVRVejFfVG9rZW46SUNxN2JIcmFNb3Z1d1F4SkdSVmNYZHVRbktkXzE3ODEzMzg5OTE6MTc4MTM0MjU5MV9WNA&add_watermark=true&scene_type=CCM)
...
1. thread_id = user_id + 随机字符串
2. 查询的时候，根据user_id的前缀查询
...
1. 登录校验，查数据库   查到user id，也会对user_id加密，返回token（不是jwt的）
...
3. 实现 SSE 流式输出与 LangGraph astream_events 深度集成，通过监听 on_chat_model_stream 事件捕获 Token 级别实时流，配合前端 ReadableStream 解析，将用户感知的首字延迟（TTFT）降低 60%；同时保证流式与非流式模式下 Checkpointer 数据结构一致性。
4. 针对图片批量生成场景实现异步并行优化，通过 asyncio.gather 配合 run_in_executor 将同步 API 封装为协程，多张配图生成由串行改为并发执行；引入指数退避重试策略（1s→2s→4s），有效应对 LLM 服务抖动，整体生成耗时缩减 70%。
5. 构建多模型动态路由机制，简单任务分发至轻量 Flash 模型，复杂任务路由至旗舰模型（Doubao-1.8）；结合 with_structured_output 实现 Pydantic Schema 强约束输出，并通过 JSON Mode + 手动解析方案解决结构化输出与 Token 统计不兼容问题，在保证输出质量的同时降低 40% 推理成本。
6. 设计环境变量驱动的 Mock 开关体系（llm_mock、image_mock、checkpointer_mock），实现开发阶段与外部服务解耦；配合 Swagger UI 可视化评测与 Ngrok 公网映射，将本地调试效率提升 3 倍，团队迭代周期从 2 天缩短至半天。
7. 接入 LangSmith 实现 Graph 全链路可观测性，自动采集节点流转路径、State 注入状态及各阶段耗时；配合 structlog 结构化日志与 request_id 关联机制，实现问题定位从小时级压缩至分钟级。
